...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 2. Netease HTTP API for avatars (no SDK needed)
    if (artists.length > 0) {
      await Promise.allSettled(artists.map(async (artist) => {
        try {
          const r = await fetch(
            `https://music.163.com/api/search/get?s=${encodeURIComponent(artist.name)}&type=100&limit=3`,
            { headers: { 'Referer': 'https://music.163.com' } }
          )
          if (!r.ok) return
          const data: any = await r.json()
          const neteaseResults = data?.result?.artists || []
          const match = neteaseResults.find((a: any) =>
            toSimplified(a.name) === artist.name
          )
          if (match?.picUrl) {
